...
bins_meas = np.linspace(0.5, 2.5, 20)
...
inner_spline_knots = np.linspace(0.5, 2.5, 15)
...
plt.plot(x, pdf, "k-", lw=1, color="red", label="true pdf (renormed)")
...
plt.plot(x, sci.splev(x, (tck), ext=1), color="red", lw=2, label="unfolded")

# Plot parameters
plt.xlim(-0.5, 3.5)
plt.xlabel("x")
plt.ylabel("Probability")
plt.title("Normed true, measured MC and unfolded pdf f(x)")
plt.legend(loc="best")
